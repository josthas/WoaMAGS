 ...
 *
 *  NOTE: If this is crashing on VS2008, I have changed al386vc.h to replace
 *  the contents of bmp_write_line and bmp_read_line with this:
 *  return (uintptr_t)&bmp->line[lyne][0];
 *  which fixes the problem. Some sort of ASM issue by the looks of it.
 */
void Cstretch_blit(BITMAP *src, BITMAP *dst, int sx, int sy, int sw, int sh,
 ...
 */
void Cmasked_stretch_blit(BITMAP *src, BITMAP *dst, int sx, int sy, int sw, int sh,
                         int dx, int dy, int dw, int dh)
{
   ASSERT(src);
   ASSERT(dst);
