...
void mgetgraphpos()
{
    ...
    if (disable_mgetgraphpos)
    {
        ...
        {
            mousex = Clamp(boundx1, boundx2, mousex);
            mousey = Clamp(boundy1, boundy2, mousey);
    ...
    if (!switched_away && Mouse::ControlEnabled)
    {
        ...
        {
            real_mouse_x = Clamp(Mouse::ControlRect.Left, Mouse::ControlRect.Right, real_mouse_x + dx);
            real_mouse_y = Clamp(Mouse::ControlRect.Top, Mouse::ControlRect.Bottom, real_mouse_y + dy);
            position_mouse(real_mouse_x, real_mouse_y);
        }
