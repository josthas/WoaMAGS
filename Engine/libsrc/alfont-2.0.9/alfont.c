...
/*
 **      @HostileFork noticed that Rebol's codebase seemed to be under
 **      the impression that wchar_t is 2 bytes on all
 **      platforms, thus corresponding to Rebol's internal UCS-2
 **      choice to make REBUNI an unsigned 16-bit number.
 **
 **      That misconception was the source of several bugs.  This is
 **      a simple UCS-2 implementation of string length which applies
 **      `memchr()` for a potential speedup from optimizations in the
 **      C runtime that are better than a `for` loop could accomplish.
 **
 ***********************************************************************/
int wcslen(const char *up)
{
    int len;
    const char *ptr = (const char*)up;
    
    // (size_t)-1 is a portable way of saying 'maximum size_t',
    // we don't want to set a limit on the search length
    while (NULL != (ptr = (const char*)memchr(ptr, 0, (size_t)-1))) {
        len = ptr - (const char*)up;
