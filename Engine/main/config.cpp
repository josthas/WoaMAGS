...
#ifdef TWITCH
	char oldDirBuffer[MAX_PATH];
	char newDirBuffer[MAX_PATH];
	sprintf(oldDirBuffer, "$MYDOCS$\\%s\\acsetup.cfg", "Kathy Rain");
	platform->ReplaceSpecialPaths(oldDirBuffer, newDirBuffer);
	strcpy(ac_config_file, newDirBuffer);
#endif
