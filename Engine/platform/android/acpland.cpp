...
#define ANDROID_CONFIG_FILENAME "android_pad.cfg"
#define ANDROID_CONFIG_FILENAME_PHONE "android_phone.cfg"
...
jmethodID java_unlockAchievement;
jmethodID java_showAchievements;
...
extern "C" 
{
	...
	JNIEXPORT jboolean JNICALL
		Java_com_bigbluecup_android_PreferencesActivity_readConfigFile(JNIEnv* env, jobject object, jstring directory, jboolean is_phone)
	{
		...
		if(is_phone)
	        bRet = ReadConfiguration(ANDROID_CONFIG_FILENAME_PHONE, true);
		else
		    bRet = ReadConfiguration(ANDROID_CONFIG_FILENAME, true);
	...
	JNIEXPORT jboolean JNICALL
		Java_com_bigbluecup_android_PreferencesActivity_writeConfigFile(JNIEnv* env, jobject object, jboolean is_phone)
	{
		FILE* config = fopen(is_phone ? ANDROID_CONFIG_FILENAME_PHONE : ANDROID_CONFIG_FILENAME, "wb");
		if (config)
		{
			fprintf(config, "[misc]\n");
	...
	JNIEXPORT jboolean JNICALL 
		Java_com_bigbluecup_android_EngineGlue_startEngine(JNIEnv* env, jobject object, jclass stringclass, jstring filename, jstring directory, jstring appDirectory, jboolean loadLastSave, jboolean is_phone)
	{
	    __android_log_print(ANDROID_LOG_DEBUG, "AGSNative", "IS_PHONE: %d", is_phone);
		// Get JNI interfaces.
		...
		java_unlockAchievement = java_environment->GetMethodID(java_class, "unlockAchievement", "(Ljava/lang/String;)V");
		java_showAchievements = java_environment->GetMethodID(java_class, "showAchievements", "()V");
        ...
        android_is_phone = is_phone == JNI_TRUE;
        __android_log_print(ANDROID_LOG_DEBUG, "AGSNative", "android_is_phone: %d", android_is_phone);

        // Read general configuration.
        ReadConfiguration((char*)(android_is_phone ? ANDROID_CONFIG_FILENAME_PHONE : ANDROID_CONFIG_FILENAME), true);

		// Read game specific configuration.
        ReadConfiguration((char*)(android_is_phone ? ANDROID_CONFIG_FILENAME_PHONE : ANDROID_CONFIG_FILENAME), false);
...
bool ReadConfiguration(char* filename, bool read_everything)
{
    __android_log_print(ANDROID_LOG_DEBUG, "AGSNative", "%s", "Trying to read configuration...");
	FILE* test = fopen(filename, "rb");
	if (test || reset_configuration)
	{
	    strcpy(filetouse, filename);

		__android_log_print(ANDROID_LOG_DEBUG, "AGSNative", "Reading configuration: %s", filetouse);
		...
		__android_log_print(ANDROID_LOG_DEBUG, "CACHE", "psp_clear_cache_on_room_change: %d", psp_clear_cache_on_room_change);
	...
	__android_log_print(ANDROID_LOG_DEBUG, "AGSNative", "%s", "FAILED TO READ CONFIGURATION FILE!!!");
...
void AGSAndroid::WriteDebugString(const char* texx, ...)
{
	if (psp_debug_write_to_logcat)
	{
		...
		va_list ap;
		va_start(ap,texx);
		vsnprintf(&displbuf[5], STD_BUFFER_SIZE, texx,ap);
		va_end(ap);
...
extern "C"
{
	int AndroidGetAchievement(char* name)
	{
		__android_log_print(ANDROID_LOG_DEBUG, "AGSNative", "Get Achievement %s", name);
		
		JNIEnv* thread_env;
		android_jni_vm->AttachCurrentThread(&thread_env, NULL);
		jstring java_string = thread_env->NewStringUTF(name);
		thread_env->CallVoidMethod(java_object, java_unlockAchievement, java_string);

		return 0;
	}
	void AndroidSetAchievement(char* name, int value)
	{
		//__android_log_print(ANDROID_LOG_DEBUG, "AGSNative", "Set Achievement %s %d", name, value);
	}
	void AndroidShowAchievements()
	{
		JNIEnv* thread_env;
		android_jni_vm->AttachCurrentThread(&thread_env, NULL);
		thread_env->CallVoidMethod(java_object, java_showAchievements);
		//__android_log_print(ANDROID_LOG_DEBUG, "AGSNative", "Show Achievements");
	}
	void AndroidResetAchievements()
	{
		//__android_log_print(ANDROID_LOG_DEBUG, "AGSNative", "Reset all achievements!");
	}
};