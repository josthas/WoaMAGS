...
namespace galaxy
{
	namespace api
	{
		...
		enum LobbyType
		{
			LOBBY_TYPE_PRIVATE, ///< Only invited users are able to join the lobby.
			LOBBY_TYPE_FRIENDS_ONLY, ///< Visible only to friends or invitees, but not in lobby list. Forbidden for the Game Server.
			LOBBY_TYPE_PUBLIC, ///< Visible for friends and in lobby list.
			LOBBY_TYPE_INVISIBLE_TO_FRIENDS ///< Returned by search, but not visible to friends. Forbidden for the Game Server.
		};
		...
		{
			DEPRECATED_LOBBY_TOPOLOGY_TYPE_FCM_HOST_MIGRATION, ///< All users are connected with each other. Disconnection of lobby owner results in choosing a new one. Deprecated: use LOBBY_TOPOLOGY_TYPE_FCM_OWNERSHIP_TRANSITION instead.			
			LOBBY_TOPOLOGY_TYPE_FCM, ///< All users are connected with each other. Disconnection of lobby owner results in closing the lobby.
			LOBBY_TOPOLOGY_TYPE_STAR, ///< All users are connected with lobby owner. Disconnection of lobby owner results in closing the lobby.
			LOBBY_TOPOLOGY_TYPE_CONNECTIONLESS, ///< All users are connected only with server. Disconnection of lobby owner results in choosing a new one. Forbidden for the Game Server.
			LOBBY_TOPOLOGY_TYPE_FCM_OWNERSHIP_TRANSITION ///< All users are connected with each other. Disconnection of lobby owner results in choosing a new one. Forbidden for the Game Server.
		};
		...
		{
			LOBBY_COMPARISON_TYPE_EQUAL, ///< The lobby should have a property of a value that is equal to the one specified.
			LOBBY_COMPARISON_TYPE_NOT_EQUAL, ///< The lobby should have a property of a value that is not equal to the one specified.
		...
		enum LobbyCreateResult
		{
			...
			LOBBY_CREATE_RESULT_CONNECTION_FAILURE ///< Unable to communicate with backend services.
		};
		...
		enum LobbyEnterResult
		{
			...
			LOBBY_ENTER_RESULT_CONNECTION_FAILURE ///< Unable to communicate with backend services.
		};
		...
		{
			LOBBY_LIST_RESULT_SUCCESS, ///< The list of lobbies retrieved successfully.
			LOBBY_LIST_RESULT_ERROR, ///< Unexpected error.
			LOBBY_LIST_RESULT_CONNECTION_FAILURE ///< Unable to communicate with backend services.
		};
		...
		public:
			 ...
			 * @param [in] result lobby list result.
			 */
		...
		public:
			...
			{
				LOBBY_LEAVE_REASON_UNDEFINED, ///< Unspecified error.
				LOBBY_LEAVE_REASON_USER_LEFT, ///< The user has left the lobby as a result of calling IMatchmaking::LeaveLobby().
				LOBBY_LEAVE_REASON_LOBBY_CLOSED, ///< The lobby has been closed (e.g. the owner has left the lobby without host migration).
				LOBBY_LEAVE_REASON_CONNECTION_LOST ///< The Peer has lost the connection.
			};
			 ...
			 *
			 * @param [in] lobbyID The ID of the lobby.
			 * @param [in] leaveReason The cause of leaving the lobby.
			 */
		 ...
		 */
		class ILobbyDataUpdateListener : public GalaxyTypeAwareListener<LOBBY_DATA_UPDATE_LISTENER>
		{
		public:
		 ...
		 */
		class ILobbyMemberDataUpdateListener : public GalaxyTypeAwareListener<LOBBY_MEMBER_DATA_UPDATE_LISTENER>
		{
		public:
		...
		public:
			 ...
			 * @param [in] lobbyCreatedListener The lobby creation listener for specific operation call[EXPERIMENTAL].
			 * @param [in] lobbyEnteredListener The lobby entering listener for specific operation call[EXPERIMENTAL].
			 */
			virtual void CreateLobby(
				...
				ILobbyEnteredListener* const lobbyEnteredListener = NULL) = 0;
			 ...
			 * @param [in] listener The listener for specific operation call [EXPERIMENTAL].
			 */
