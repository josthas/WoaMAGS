...
namespace galaxy
{
	namespace api
	{
		...
		enum NatType
		{
			NAT_TYPE_NONE, ///< There is no NAT at all.
			NAT_TYPE_FULL_CONE, ///< Accepts any datagrams to a port that has been previously used.
			NAT_TYPE_ADDRESS_RESTRICTED, ///< Accepts datagrams to a port if the datagram source IP address belongs to a system that has been sent to.
			NAT_TYPE_PORT_RESTRICTED, ///< Accepts datagrams to a port if the datagram source IP address and port belongs a system that has been sent to.
			NAT_TYPE_SYMMETRIC, ///< A different port is chosen for every remote destination.
			NAT_TYPE_UNKNOWN ///< NAT type has not been determined.
		};
		...
		enum ConnectionType
		{
			CONNECTION_TYPE_NONE, ///< User is connected only with server.
			CONNECTION_TYPE_DIRECT, ///< User is connected directly.
			CONNECTION_TYPE_PROXY ///< User is connected through a proxy.
		};
